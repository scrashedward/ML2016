...
w_input = np.random.rand(nNum,58)
w = np.random.rand(nNum + 1)
alpha = 0.3 #learning rate
iternum = 20000; #iteration number
...
#	print o_input.shape
	o = logistic(np.column_stack((o_input, np.zeros(4001) + 1)).dot(w))
#	print o.shape
	delta_o = ((o-y)*o*(1-o))
#	print delta_o.shape

	gdw = (np.column_stack((o_input, np.zeros(4001) + 1)).T * delta_o.T).mean(axis = 1)
#	print gdw.shape

	gdwSum = gdwSum + np.square(gdw)
	w = w - alpha * gdw / np.sqrt(gdwSum +eta)

	gdwi = ((train_data.T * delta_o.T).dot(o_input * ( 1 - o_input)))*w[:nNum]/4001
	gdwi = gdwi.T
...
test_data = np.column_stack((test_data, np.zeros(600) + 1))

o_input = logistic(test_data.dot(w_input.T))

test_o = logistic(np.column_stack((o_input, np.zeros(600))).dot(w))
